...
import { getViewportOffset, nextTick } from '@/utils/dom';
...
export function useContentHeight(
  ...
  subtractHeightRefs: RefObject<Element>[],
  subtractSpaceRefs: RefObject<Element>[],
  upwardSpace: Upward = 0,
  offsetHeightRef: number = 0,
) {
  const [contentHeight, setContentHeight] = useState<Nullable<number>>(null);
  const { footerHeight: layoutFooterHeight } = useLayoutHeight();
  ...
  function getEl(element: any): Nullable<HTMLDivElement> {
    ...
    return (element instanceof HTMLDivElement ? element : element.current) as HTMLDivElement;
  }
  ...
  async function calcContentHeight() {
    ...
    await nextTick();
    const anchorEl = getEl(anchorRef);
    if (!anchorEl) {
      return;
    }
    ...
    const calcCompensationHeight = () => {
      compensationHeight.elements?.forEach((item) => {
        height += getEl(item)?.offsetHeight ?? 0;
      });
    };
    // if (compensationHeight.useLayoutFooter && layoutFooterHeight > 0) {
